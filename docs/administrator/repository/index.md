...
<img src="https://kartoza.github.io/qgis-plugin-template/user/guide/img/icon_loading.gif" style="align-content: center; background:transparent;"/>
</div>
<div>
...
"GET /repos/kartoza/qgis-plugin-template/pulls",
{'state':'all'}
);

const fetched_artifacts = await octokit.request(
"GET /repos/kartoza/qgis-plugin-template/actions/artifacts",
{'per_page': 30}
);
...
for ( const pull of pulls.data ){
    ...
    for ( const artifact of fetched_artifacts.data.artifacts){
        ...
        if ( artifact['workflow_run']['head_sha'] == head_sha &&
            artifact['name'].indexOf('qgis_plugin_template') != -1 ){
...
for ( const artifact of fetched_artifacts.data.artifacts){
    ...
    if ( artifact['name'].indexOf('qgis_plugin_template') == -1){
        continue;
    }
    const commit = await octokit.request(
    "GET /repos/kartoza/qgis-plugin-template/commits/"+
...
"https://raw.githubusercontent.com/kartoza/qgis-plugin-template/docs/docs/admin/artifacts_list.txt"
);
